...
def _dropzone(upload) -> None:
    ...
    # click handler bound in JavaScript: not focusable, no role, no accessible
    # name. The file picker was reachable only through a 34x34 anchor inside
    # the opacity:0 uploader below, which is invisible - focus went somewhere
    # the user cannot see.
    #
    # role=button plus tabindex=0 makes the visible affordance the focusable
    # control, and the keydown handler further down gives it Enter and Space.
    ...
    ui.timer(
        ...
        lambda: ui.run_javascript(
            ...
            # must. preventDefault stops Space from scrolling the page instead.
            ...
            "};"
            # The uploader is hidden with opacity: 0, but its own pick-files
            # anchor stayed in the tab order - an invisible focus stop on the
            # primary upload path (finding F-49). Take its focusable
            # descendants out of the tab order; the dropzone above is the
            # control now.
            #
            # getElement() returns the Vue component, not a DOM node, so the
            # element has to be reached through $el. This runs last and is
            # guarded: if it ever fails it must not take the drag-and-drop
            # listeners or the progress interval above down with it.
            "try {"
            "  const uplEl = upl.$el ||"
...
def table_upload(table, mode: str = "files") -> None:
    ...
    dialog_name = "Record audio" if mode == "record" else "Upload files"
    with ui.dialog().props(f'aria-label="{dialog_name}"') as dialog:
