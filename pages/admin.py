                ...
                with ui.column().style("flex: 1;"):
                    ...
                    with ui.row().classes("w-full gap-8"):
                        with ui.column().style("min-width: 30%;"):
                            ...
                            ui.label(
                                f"Transcribed minutes via Sunet Scribe: {self.stats.get('total_transcribed_minutes', 0)}"
                            )
                        ...
                        with ui.column():
                            ...
                            ui.label(f"Transcribed minutes via Sunet Scribe: {self.stats.get('total_transcribed_minutes_last_month', 0)}").classes("text-sm")
                ...
                with ui.column().style("flex: 0 0 auto;"):
                    ...
                    statistics.on(
                        ...
                        lambda: ui.navigate.to(
                            f"/admin/customers/stats/{self.customer_id}"
                        ),
                    )
...
# Update the existing customers route to redirect
@ui.page("/admin/customers")
def customers_redirect() -> None:
    customers()

@ui.refreshable
@ui.page("/admin/customers/stats/{customer_id}")
def customer_statistics(customer_id: str) -> None:
    """
    Page to show statistics of a customer with improved layout and design.
    """
    page_init()
    ...
    try:
        ...
        stats = stats_res.json()["result"]

    except requests.RequestException as e:
        ui.label(f"Error fetching customer statistics: {e}").classes("text-lg text-red-500 text-center mt-6")
        return
    ...
    with ui.element("div").classes("stats-container w-full"):
        with ui.element("div").classes("stats-card w-full"):
            ...
            if customer['priceplan'] == 'fixed' and customer.get('blocks_purchased', 0) > 0:
                ui.separator().classes("my-4")
                ui.label("Block Usage (Fixed Plan)").classes("text-xl font-bold text-gray-800")
                blocks_purchased = customer.get('blocks_purchased', 0)
                ...
                ui.label(f"Blocks purchased: {blocks_purchased} ({minutes_included:,} minutes)").classes("text-lg text-gray-600")
                ui.label(f"Blocks consumed: {blocks_consumed:.2f}").classes("text-lg text-blue-600 font-semibold")

                if overage_minutes > 0:
                    ui.label(f"⚠️ Overage minutes: {overage_minutes:,} minutes").classes("text-lg text-red-600 font-bold")
                    ui.label(f"Total usage: {stats.get('total_transcribed_minutes', 0):,} minutes (exceeding purchased blocks)").classes("text-md text-red-500")
                else:
                    ui.label(f"✓ Remaining minutes: {remaining_minutes:,} minutes").classes("text-lg text-green-600 font-semibold")
                    ui.label(f"Total usage: {stats.get('total_transcribed_minutes', 0):,} minutes").classes("text-md text-gray-500")

            ui.separator().classes("my-4")
            ui.label(f"Total users: {stats.get('total_users', 0)}").classes("text-lg text-gray-600")
            ui.label(f"Transcribed files this month: {stats.get('transcribed_files', 0)} files").classes("text-lg text-gray-600")
            ui.label(f"Transcribed files last month: {stats.get('transcribed_files_last_month', 0)} files").classes("text-lg text-gray-600")
            ui.label(f"Transcribed minutes this month: {stats.get('total_transcribed_minutes', 0):.0f} minutes").classes("text-lg text-gray-600")
            ui.label(f"Transcribed minutes last month: {stats.get('total_transcribed_minutes_last_month', 0):.0f} minutes").classes("text-lg text-gray-600")

    with ui.footer().style("background-color: #ffffff;"):
        with ui.row().style("justify-content: flex-left; width: 100%; padding: 16px; gap: 8px;"):
            ui.button("Back to customers").classes("button-close").props(
                "color=black flat"
            ).style("width: 150px").on("click", lambda: ui.navigate.to("/admin/customers"))